...
def _rsf_deserialize(parameters):
    """RSF's own quirk (preserved as-is): each client ndarray gets flattened to
    raw bytes before the aggregator sees it -- RSFAggregator.aggregate() just
    concatenates client trees, it never reads BaseFLStrategy's computed weights
    (unlike cox's), so this migration is behavior-preserving regardless of
    smoothing settings, not just with smoothing off."""
...
class CustomStrategy(BaseFLStrategy):
    def __init__(self, rounds: int, saving_path :str = '/sandbox/', **kwargs):
        super().__init__(
            aggregator_cls=RSFAggregator,
            serialize_fn=fl.common.ndarrays_to_parameters,
            deserialize_fn=_rsf_deserialize,
            **kwargs,
        )
        self.rounds = rounds
    ...
    def aggregate_fit(self, server_round: int, results, failures):
        """Reuses BaseFLStrategy's merge; adds rsf's own "save the global model
        ...
        if server_round == self.rounds:
            ...
            with open(f"{self.saving_path}/models/rsf.pkl", "wb") as f:
                pickle.dump(model, f)
    ...
    def aggregate_evaluate(
        ...
        self.results_history['MODEL_TYPE'] = 'rsf'
...
def get_fit_config_fn(estimators, random_state=42):
    def fit_config(rnd: int):
        conf = {"model_type": 'rsf', "n_estimators": estimators, "random_state": random_state}
        return conf
    return fit_config
